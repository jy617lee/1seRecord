...
class Config(context: Context) : BaseConfig(context) {
    ...
    var savePhotosFolder: String
        get(): String {
            var path = prefs.getString(SAVE_PHOTOS, Environment.getExternalStorageDirectory().absolutePath + File.separator + "videoDiary")
            if (!File(path).exists() || !File(path).isDirectory) {
                path = Environment.getExternalStorageDirectory().absolutePath + File.separator + "videoDiary"
